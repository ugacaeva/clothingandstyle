...
function openProductModal(productId) { 
    // Выполнение AJAX-запроса для получения информации о товаре 
    $.ajax({ 
        url: 'get_product_info.php', // Путь к PHP-скрипту для получения информации о товаре 
        method: 'POST', 
        data: { productId: productId }, 
        success: function(response) { 
            // Обработка успешного ответа от сервера 
            var productInfo = JSON.parse(response); // Преобразование JSON-строки в объект 
